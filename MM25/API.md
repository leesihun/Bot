...
Receives event notifications from Huni Messenger. This endpoint is registered automatically on startup — you do not need to call it manually.
...
# Send a message to Hoonbot via webhook
def send_webhook(source: str, message: str, secret: str = None):
    url = f"http://localhost:3939/webhook/incoming/{source}"
    headers = {"Content-Type": "application/json"}
    if secret:
        headers["X-Webhook-Secret"] = secret
    
    response = httpx.post(url, json={"message": message}, headers=headers)
    return response.json()

# Example usage
send_webhook("github", "New issue opened: Bug in login", "my-secret")
```
...
async function sendWebhook(source, message, secret = null) {
  const url = `http://localhost:3939/webhook/incoming/${source}`;
  const headers = {
    'Content-Type': 'application/json'
  };
  if (secret) {
...
// Example usage
sendWebhook('github', 'New PR merged', 'my-secret')
  .then(console.log)
  .catch(console.error);
```
...
curl -X POST http://localhost:3939/webhook/incoming/github \
  -H "Content-Type: application/json" \
  -d '{"message": "Build passed", "branch": "main"}'

# With secret
curl -X POST http://localhost:3939/webhook/incoming/monitor \
  -H "Content-Type: application/json" \
  -H "X-Webhook-Secret: your-secret" \
  -d '{"message": "CPU usage at 95%", "host": "server-1"}'
```
...
There are no explicit rate limits, but the following internal mechanisms control throughput:

- **Message debouncing**: 1.5 second window per room
- **LLM retries**: Up to 3 attempts with exponential backoff
- **Heartbeat**: Configurable interval (default 3600 seconds)
...
```bash
curl -X POST http://localhost:3939/webhook \
  -H "Content-Type: application/json" \
  -d '{
    "event": "new_message",
    "roomId": 1,
...
```bash
curl -X POST http://localhost:3939/webhook/incoming/test \
  -H "Content-Type: application/json" \
  -d '{"message": "Test message from external service"}'
```
